...
from arakis.database.models import Workflow, Manuscript, Paper
...
async def export_manuscript_json(
    ...
    result = await db.execute(
        select(Manuscript).where(Manuscript.workflow_id == workflow_id)
    )
...
async def export_manuscript_markdown(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=manuscript_{workflow_id}.md"
        },
    )
...
async def export_manuscript_pdf(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=manuscript_{workflow_id}.pdf"
            },
        )
...
async def export_manuscript_docx(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=manuscript_{workflow_id}.docx"
            },
        )
    except FileNotFoundError:
        raise HTTPException(
